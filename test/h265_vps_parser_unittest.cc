...
TEST_F(H265VpsParserTest, TestSampleVPS2) {
  ...
  EXPECT_THAT(vps_->vps_max_latency_increase_plus1,
              ::testing::ElementsAreArray({0}));
  EXPECT_EQ(5, vps_->vps_max_layer_id);
  EXPECT_EQ(2, vps_->vps_num_layer_sets_minus1);
  EXPECT_EQ(2, vps_->layer_id_included_flag.size());
  EXPECT_THAT(vps_->layer_id_included_flag[0],
              ::testing::ElementsAreArray({0, 0, 0, 0, 0, 0}));
  EXPECT_THAT(vps_->layer_id_included_flag[1],
