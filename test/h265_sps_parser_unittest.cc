...
class H265SpsParserTest : public ::testing::Test {
 public:
  ...
  std::shared_ptr<H265SpsParser::SpsState> sps_;
};
...
TEST_F(H265SpsParserTest, TestSampleSPS) {
  ...
  EXPECT_EQ(0,
            sps_->profile_tier_level->general->max_422chroma_constraint_flag);
  EXPECT_EQ(0,
            sps_->profile_tier_level->general->max_420chroma_constraint_flag);
  EXPECT_EQ(0,
            sps_->profile_tier_level->general->max_monochrome_constraint_flag);
  EXPECT_EQ(0, sps_->profile_tier_level->general->intra_constraint_flag);
  EXPECT_EQ(
      0, sps_->profile_tier_level->general->one_picture_only_constraint_flag);
  EXPECT_EQ(0,
            sps_->profile_tier_level->general->lower_bit_rate_constraint_flag);
  ...
  EXPECT_THAT(sps_->sps_max_num_reorder_pics, ::testing::ElementsAreArray({0}));
